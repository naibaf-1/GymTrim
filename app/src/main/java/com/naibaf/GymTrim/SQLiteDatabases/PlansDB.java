...
public class PlansDB extends SQLiteOpenHelper {
    ...
    // Get all data of a specific plan at once
    ...
    //-------------------------------------------------------------------------------
    // Overlay showing further information about an exercise
    //-------------------------------------------------------------------------------
    // Get exercise Id
    ...
    public Cursor getBasicExerciseData(int exerciseId){
        return DB.rawQuery(" Select * From PlansDetails p INNER JOIN Exercise e ON p.plansId = e.ExerciseID Where e.exerciseMainId = ? ", new String[]{String.valueOf(exerciseId)});
    }
    ...
    // Get the id of an exercise in EDB in order to load changes by the editor
    ...
    // Update the values of an exercise while training
